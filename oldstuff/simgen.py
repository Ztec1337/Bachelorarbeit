...
class sim():
    ...
    def generate(self,numberofSpectra,aFieldStrength,b,c,ODscaler,CoupledHamiltonian,randomizeFieldstrength = True,randomizePhase = True,randomizeHamiltonian = False, randomizeODscaler = False ):
        ...
        aFieldStrengths,bs,cs,ODscalers = self.getparams(numberofSpectra,aFieldStrength,b,c,ODscaler,randomizeFieldstrength,randomizePhase, randomizeODscaler,randomizeHamiltonian)
        ...
        for indx,(aFieldStrength,b,c,ODscaler) in tqdm(enumerate(zip(aFieldStrengths,bs,cs,ODscalers)), total=numberofSpectra):
            ...
            self.data.append([self.dt,self.steps,self.stepsCont,self.delay,self.frequency,self.spectralwidth,aFieldStrength,b,c,ODscaler,dim,CoupledHamiltonian.real.tolist(),spectrum.tolist(),optdensity.tolist()])
            ...
            if indx%5 == 0 : 
                ...
                temp.to_hdf('dataset.h5', key='all', mode='a')  
                
                #database = database.append(temp,ignore_index=True)
                #database.to_csv("dataset.csv",index=False,header = ["dt", "steps", "stepsCont","delay","frequency","spectralwidth","aFieldStrength","b","c","ODscaler","dim","CoupledHamiltonian","spectrum","optdensity"]) 
                self.data = []
    ...
    def getparams(self,numberofSpectra,aFieldStrength,b,c,ODscaler,randomizeFieldstrength,randomizePhase, randomizeODscaler,randomizeHamiltonian):
